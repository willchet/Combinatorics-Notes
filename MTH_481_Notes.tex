...
\begin{theorem}[Wilf Rules]\label{wilf}
\;
\begin{enumerate}[label=\Roman*.]
\item\label{wilf1} If $F(x)\ogf \{f_n\}$, then for $k\geq0$, $\displaystyle\{f_{n+k}\}\ogf \frac{F(x)-f_0-f_1x-\cdots-f_{k-1}x^{k-1}}{x^k}$
\item\label{wilf2} If $F(x)\ogf \{f_n\}$ and $P$ is a polynomial, then $\{P(n)a_n\}\ogf  P(xD)f$, where $D$ is the derivative operator with respect to $x$
...
\begin{example}
Suppose $a_n=n^2$. Then by Wilf II and Lemma \ref{generating_func_recip_linear}:
\begin{equation*}
A(x)
=(xD)^2\frac1{1-x}
...
\begin{example}
Consider the Fibonacci numbers, with the recurrence $f_{n+2}=f_{n+1}+f_n$ with initial conditions $f_0=f_1=1$. We first note that these have a combinatorial interpretation; namely, $f_n$ gives the number of ways to tile a $1\times n$ grid with monominoes and dominoes. Indeed, the result follows by conditioning on whether the leftmost square is covered by a monomino ($f_{n+1}$ ways to tile the remaining squares) or covered by a domino ($f_n$ ways to tile the remaining squares).

\medskip

Another way to derive the recurrence from the combinatorial interpretation is to use generating function. Note that $A_1(x)=x^1+x^2$ is a generating function whose coefficients on $x^n$ describes the number of ways to tile a $1\times n$ grid with just one tile. Then the number of ways with $j$ tiles is given by the coefficients of $A_j(x)=(x^1+x^2)^j$. So, using composition:
\begin{equation*}
F(x)
=\sum_{j=0}^\infty A_j(x)
=\sum_{j=0}^\infty(x+x^2)^j
=\frac1{1-(x+x^2)}
=\frac1{1-x-x^2}
\end{equation*}

Thus, the recurrence is $f_{n+2}=f_{n+1}+f_n$ by Theorem \ref{gen_shortcut}.

\medskip

We now find a closed-form for the Fibonacci numbers. $\frac{F(x)-f_0-f_1x}{x^2}=\frac{F(x)-f_0}{x}+F(x)$ by Wilf I, so solving yields:
\begin{equation*}
F(x)
=\frac1{1-x-x^2}
=-\frac1{(x-\frac{-1+\sqrt5}2)(x-\frac{-1-\sqrt5}2)}
=\frac1{\sqrt5}\left(\frac1{x+\frac{1+\sqrt5}2}-\frac1{x-\frac{-1+\sqrt5}2}\right)
\end{equation*}

Thus, by Lemma \ref{easier_gen_linear}, we have:

\begin{equation*}
f_n
=\frac1{\sqrt5}\left[\left(\frac{-1+\sqrt5}2\right)^{-n-1}-\left(-\frac{1+\sqrt5}2\right)^{-n-1}\right]
=\frac1{\sqrt5}\left[\left(\frac{1+\sqrt5}2\right)^{n+1}-\left(\frac{1-\sqrt5}2\right)^{n+1}\right]
\end{equation*}
\end{example}
